...
testset <- read.csv("pml-training.csv",na.strings=c("","NA"),row.names=1)
...
Try recursive partitioning using all predictors. The accuracy of this method is not very high - just as good as a random coin toss.
...
Calculate out of sample error using test data set. Random Forest method is used because of it's high accuracy during training phase. This doesn't disappoint and produces good accuracy rates on the test data set.

```{r echo=TRUE}
p <- predict(fit2,testset)
print(confusionMatrix(p,testset$classe)$overall[1])
```
